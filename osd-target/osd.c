...
static inline uint8_t get_obj_type(struct osd_hndl *osd,
 ...
 */
static int get_ccap(struct osd_hndl *osd, void *outbuf, uint64_t outlen,
 ...
 * ==0: OSD_OK (success), used_outlen stores consumed outbuf len
 */
static int get_ccap_aslist(struct osd_hndl *osd, uint32_t number,
 ...
 * ==0: OSD_OK (success), used_outlen stores consumed outbuf len
 */
static int get_utsap(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
 ...
 */
static int get_utsap_aslist(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
 ...
 * OSD_OK: on success, sets used_outlen
 */
static int get_uiap(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
 ...
 * !=0: failure
 */
static int osd_initialize_db(struct osd_hndl *osd)
{
	int i = 0;
	int ret = 0;
	uint64_t pid = 0;
...
int osd_open(const char *root, struct osd_hndl *osd)
{
	int ret;
	char path[MAXNAMELEN];
...
int osd_close(struct osd_hndl *osd)
{
	int ret;
 ...
 * Commands
 */
int osd_append(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
...
static int osd_create_datafile(struct osd_hndl *osd, uint64_t pid,
			       uint64_t oid)
{
	int ret = 0;
	char path[MAXNAMELEN];
...
static inline void osd_remove_tmp_objects(struct osd_hndl *osd, uint64_t pid,
 ...
 */
int osd_create(struct osd_hndl *osd, uint64_t pid, uint64_t requested_oid,
...
int osd_create_and_write(struct osd_hndl *osd, uint64_t pid,
...
/* osd2r01 sec. 6.5 */
int osd_create_collection(struct osd_hndl *osd, uint64_t pid,
...
int osd_create_partition(struct osd_hndl *osd, uint64_t requested_pid,
			 uint8_t *sense)
{
	int ret = 0;
	uint64_t pid = 0;
...
int osd_flush(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
...
int osd_flush_collection(struct osd_hndl *osd, uint64_t pid, uint64_t cid,
			 int flush_scope, uint8_t *sense)
{
	osd_debug(__func__);
	return osd_error_unimplemented(0, sense);
...
int osd_flush_osd(struct osd_hndl *osd, int flush_scope, uint8_t *sense)
{
	osd_debug(__func__);
	return osd_error_unimplemented(0, sense);
}
...
int osd_flush_partition(struct osd_hndl *osd, uint64_t pid, int flush_scope,
			uint8_t *sense)
{
	osd_debug(__func__);
	return osd_error_unimplemented(0, sense);
 ...
 */
int osd_format_osd(struct osd_hndl *osd, uint64_t capacity, uint8_t *sense)
{
	int ret;
	char *root;
	char path[MAXNAMELEN];
...
static inline int
mutiplex_getattr_list(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
 ...
 */
int osd_getattr_list(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
		     uint32_t page, uint32_t number, uint8_t *outbuf,
		     uint32_t outlen, uint8_t isembedded, uint8_t listfmt,
		     uint32_t *used_outlen,  uint8_t *sense)
{
 ...
 *  >0: failed, sense set accordingly
 */
int osd_getattr_page(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
...
int osd_get_member_attributes(struct osd_hndl *osd, uint64_t pid,
 ...
 */
int osd_list(struct osd_hndl *osd, uint64_t pid, uint32_t list_id,
...
int osd_list_collection(struct osd_hndl *osd, uint64_t pid, uint64_t cid,
...
int osd_query(struct osd_hndl *osd, uint64_t pid, uint64_t cid,
 ...
 * > 0: error, sense is set
 */
int osd_read(struct osd_hndl *osd, uint64_t pid, uint64_t oid, uint64_t len,
...
int osd_remove(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
	       uint8_t *sense)
{
	int ret = 0;
	char path[MAXNAMELEN];
...
int osd_remove_collection(struct osd_hndl *osd, uint64_t pid, uint64_t cid, int force_removal,
			  uint8_t *sense)
{
	osd_debug(__func__);
	return osd_error_unimplemented(0, sense);
...
int osd_remove_member_objects(struct osd_hndl *osd, uint64_t pid,
			      uint64_t cid, uint8_t *sense)
{
	osd_debug(__func__);
	return osd_error_unimplemented(0, sense);
 ...
 */
int osd_remove_partition(struct osd_hndl *osd, uint64_t pid, uint8_t *sense)
{
	int ret = 0;
 ...
 */
int osd_set_attributes(struct osd_hndl *osd, uint64_t pid, uint64_t oid,
...
int osd_set_key(struct osd_hndl *osd, int key_to_set, uint64_t pid,
...
int osd_set_master_key(struct osd_hndl *osd, int dh_step, uint64_t key,
...
int osd_set_member_attributes(struct osd_hndl *osd, uint64_t pid,
			      uint64_t cid, uint8_t *sense)
{
	osd_debug(__func__);
	return osd_error_unimplemented(0, sense);
 ...
 */
int osd_write(struct osd_hndl *osd, uint64_t pid, uint64_t oid, uint64_t len,
