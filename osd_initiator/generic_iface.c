...
{
	memset(cmd, 0, sizeof(struct osd_command));
	cmd->cdb_len = OSD_CDB_SIZE;

	gen_osd_debug(5, "%s: Formating device %ld bytes", __func__, capacity);
	return set_cdb_osd_format_osd(cmd->cdb, capacity);
}
...
int gen_osd_set_create_partition(struct osd_command *cmd, uint64_t pid){
	...
	return set_cdb_osd_create_partition(cmd->cdb, pid);
}
			...
			uint64_t oid, uint16_t count)
{
	...
	return set_cdb_osd_create(cmd->cdb, pid, oid, count);
			...
			uint64_t oid)
{
	...
	return set_cdb_osd_remove(cmd->cdb, pid, oid);
			...
			void *buf, size_t count, size_t offset)
{
	...
	cmd->outdata = buf;
	cmd->outlen = count;
	return set_cdb_osd_write(cmd->cdb, pid, oid, count, offset);
			...
			void *buf, size_t count, size_t offset)
{
	...
	gen_osd_debug(5, "%s: Reading %ld bytes at offset %ld of %ld.%ld",
		...
		cmd->outdata = buf;
		cmd->outlen = count;
		return set_cdb_osd_read(cmd->cdb, pid, oid, count, offset);
