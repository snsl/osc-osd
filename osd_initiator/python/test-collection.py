...
command.attr_build(OSDAttr(ATTR_SET, COLLECTIONS_PG, 1, htonl(cid1)))
run(command)
command = OSDCommand.set_attributes(pid, oid)
command.attr_build(OSDAttr(ATTR_SET, COLLECTIONS_PG, 2, htonl(cid2)))
run(command)
# replace existing one
command = OSDCommand.set_attributes(pid, oid)
command.attr_build(OSDAttr(ATTR_SET, COLLECTIONS_PG, 2, htonl(cid3)))
run(command)
# delete one
command = OSDCommand.set_attributes(pid, oid)
command.attr_build(OSDAttr(ATTR_SET, COLLECTIONS_PG, 2, None))
run(command)
# set same collection twice
command = OSDCommand.set_attributes(pid, oid)
command.attr_build(OSDAttr(ATTR_SET, COLLECTIONS_PG, 3, htonl(cid1)))
runfail(command)
# remove last one
command = OSDCommand.set_attributes(pid, oid)
command.attr_build(OSDAttr(ATTR_SET, COLLECTIONS_PG, 1, None))
runfail(command)
