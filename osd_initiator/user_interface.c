...
int format_osd(int fd, int cdb_len, int capacity)
{
	...
	osd_info("response key %lx error %d,sense len %d", resp.key, resp.error, resp.sense_buffer_len); 
		...
		uint64_t buf_len, uint64_t offset, const char *buf)
{
	...
	err = dev_osd_wait_response(fd, &resp);
	osd_info("argument: '%s'", buf);
	osd_info("response key %lx error %d,sense len %d", resp.key,
		...
		uint64_t buf_len, uint64_t offset, char *bufout)
{
	...
	err = dev_osd_wait_response(fd, &resp);
	osd_info("argument: '%s'", bufout);
	osd_info("response key %lx error %d, bufout %s, sense len %d", resp.key,
...
int inquiry_osd(int fd)
{
	...
	osd_hexdump(inquiry_rsp, sizeof(inquiry_rsp));
	fflush(0);
